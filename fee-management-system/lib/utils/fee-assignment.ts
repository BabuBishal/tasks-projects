...
import { getAcademicYearForSemester, getSemesterStartDate, getDueDate } from './utils'
...
export async function assignFeeForSemester(
  ...
  try {
    ...
    // Calculate dates
    const academicYear = getAcademicYearForSemester(student.joinedYear, semester)
 ...
 */
export async function promoteSemester(studentId: string): Promise<FeeAssignmentResult> {
  try {
    const student = await prisma.student.findUnique({
      where: { id: studentId },
      include: {
    ...
    if (nextSemester > student.program.duration) {
      ...
      const totalOutstanding = student.fees.reduce((sum, fee) => sum + fee.balance, 0)
 ...
 */
export async function updateFeeStatus(feeId: string, status: string): Promise<boolean> {
  try {
